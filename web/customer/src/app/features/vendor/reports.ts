...
import { DemandPrediction, TripReportRow, VendorReportSummary } from '../../core/models';
  ...
  template: `
    ...
    <div class="grid gap-6 lg:grid-cols-3">
      <section class="lg:col-span-2">
        <h2 class="mb-2 font-semibold text-slate-900">Per-trip</h2>
        @if (loading()) {
          <p class="text-slate-500">Loading…</p>
        } @else if (rows().length === 0) {
          <p class="rounded-lg bg-slate-100 p-4 text-slate-600">No trips in this range.</p>
        } @else {
...
export class VendorReportsComponent implements OnInit {
  ...
  protected readonly rows = signal<TripReportRow[]>([]);
  ...
  protected load(): void {
    ...
    this.api.tripReport(from, to).subscribe({
  ...
  protected download(format: ReportFormat): void {
    const { from, to } = this.range();
    this.api.exportTripReport(format, from, to).subscribe({
      next: (blob) => {
