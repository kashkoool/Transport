  ...
  template: `
    ...
    <div class="grid gap-6 lg:grid-cols-3">
      <section class="rounded-xl border border-slate-200 bg-white p-4">
        <h2 class="mb-3 font-semibold text-slate-900">New cash booking</h2>
        <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-3">
          ...
          <button type="submit" [disabled]="submitting()" class="w-full rounded-md bg-emerald-600 px-4 py-2 font-medium text-white hover:bg-emerald-700 disabled:opacity-50">
            {{ submitting() ? 'Selling…' : 'Sell ticket (cash)' }}
          </button>
        </form>
      </section>

      <section class="lg:col-span-2">
        <h2 class="mb-2 font-semibold text-slate-900">Recent company bookings</h2>
        @if (loading()) {
          <p class="text-slate-500">Loading…</p>
        } @else if (bookings().length === 0) {
          <p class="rounded-lg bg-slate-100 p-4 text-slate-600">No bookings yet.</p>
        } @else {
          <div class="space-y-2">
                ...
                <div class="flex items-center gap-3">
                  ...
                  @if (b.status !== 'Cancelled') {
                    @if (confirmingCancel() === b.bookingId) {
                      <button type="button" [disabled]="busy() === b.bookingId" (click)="cancel(b)" class="text-sm font-medium text-rose-700">Confirm</button>
                      <button type="button" (click)="confirmingCancel.set(null)" class="text-sm text-slate-500">Keep</button>
                    } @else {
                      <button type="button" (click)="confirmingCancel.set(b.bookingId)" class="text-sm font-medium text-rose-600 hover:text-rose-700">Cancel</button>
                    }
                  }
                </div>
              </div>
...
export class VendorDeskComponent implements OnInit {
  ...
  protected submit(): void {
    if (this.form.invalid) {
      ...
      this.toasts.info('Please complete the trip, customer email and passenger details.');
      return;
    }
    ...
    this.api.counterBooking(body).subscribe({
      next: (r) => {
        ...
        this.toasts.success(`Booking ${r.reference} confirmed (${r.totalAmount} ${r.currency}).`);
