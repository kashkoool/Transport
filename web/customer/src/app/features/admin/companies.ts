    ...
    <div class="grid gap-6 lg:grid-cols-3">
      <section class="lg:col-span-2">
        <div class="mb-3 flex items-center gap-2 text-sm">
          ...
          <button type="button" (click)="setFilter(null)" [class.font-semibold]="filter() === null" [class.text-brand-700]="filter() === null" class="text-slate-500 hover:text-slate-800">All</button>
          @for (s of statuses; track s) {
            <button type="button" (click)="setFilter(s)" [class.font-semibold]="filter() === s" [class.text-brand-700]="filter() === s" class="text-slate-500 hover:text-slate-800">{{ s }}</button>
          }
        </div>
              ...
              <div class="rounded-xl border border-slate-200 bg-white p-4">
                ...
                @if (managerFor() === c.id) {
                  <form [formGroup]="managerForm" (ngSubmit)="createManager(c)" class="mt-3 space-y-2 rounded-lg bg-slate-50 p-3">
                    <input type="text" formControlName="fullName" placeholder="Manager full name" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
                    <input type="email" formControlName="email" placeholder="Manager email" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
                    <input type="password" formControlName="password" placeholder="Temp password" autocomplete="new-password" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
                    <p class="text-xs text-slate-500">Min 10 chars with upper, lower, digit, and a symbol.</p>
                    <button type="submit" [disabled]="busyId() === c.id" class="rounded-md bg-brand-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-brand-700 disabled:opacity-50">Create manager</button>
                  </form>
                }

                @if (notifyFor() === c.id) {
                  <form [formGroup]="notifyForm" (ngSubmit)="notify(c)" class="mt-3 space-y-2 rounded-lg bg-slate-50 p-3">
                    <input type="text" formControlName="title" placeholder="Title" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500" />
                    <textarea formControlName="message" rows="2" placeholder="Message to the company's manager(s)" class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"></textarea>
                    <button type="submit" [disabled]="busyId() === c.id" class="rounded-md bg-brand-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-brand-700 disabled:opacity-50">Send notification</button>
                  </form>
                }
              </div>
            }
