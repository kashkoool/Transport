...
@Component({
  ...
  template: `
    <h1 class="mb-6 text-2xl font-bold text-slate-900">My account</h1>
    ...
    @if (loading()) {
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        <section class="rounded-xl border border-slate-200 bg-white p-5">
          <h2 class="mb-3 font-semibold text-slate-900">Profile</h2>
          <p class="mb-3 text-sm text-slate-500">Signed in as <span class="font-medium text-slate-700">{{ p.email }}</span></p>
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-5">
          <h2 class="mb-3 font-semibold text-slate-900">Change password</h2>
...
export class ProfileComponent implements OnInit {
  ...
  protected changePassword(): void {
    if (this.passwordForm.invalid) {
      ...
      this.toasts.info('Your new password must meet the policy.');
      return;
    }
